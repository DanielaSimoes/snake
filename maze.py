...
class Maze:
    ...
    def toNetwork(self):
        p = json.dumps({'walls': self.obstacles, 'playerpos': self.playerpos, 'foodpos': self.foodpos})
        return p 
    def fromNetwork(self, p):
        s = json.loads(p)
        #JSON can't encode tuples, it encodes it as lists... lets fix it:
        self.obstacles = [(e[0], e[1]) for e in s['walls']]
        self.playerpos = [(e[0], e[1]) for e in s['playerpos']]
        self.foodpos = s['foodpos'][0], s['foodpos'][1]
    def __str__(self):
        return "walls = {}  playerpos = {}  foodpos = {}".format(str(self.obstacles), str(self.playerpos), str(self.foodpos))
